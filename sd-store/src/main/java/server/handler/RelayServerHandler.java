    ...
    public boolean handleMessage(SOAPMessageContext smc) {
        ...
        if (outbound) {
            ...
            try {
                ...
                System.out.printf("%s put token '%s' on response message header%n", CLASS_NAME, newValue);
            ...
            try {
                ...
                System.out.printf("%s got '%s'%n", CLASS_NAME, tag);

                // *** #5 ***
                
               
                System.out.printf("%s put token '%s' on request context%n", CLASS_NAME, "ACK"); // NECESSARIO??
                smc.put(REQUEST_PROPERTY, tag);
